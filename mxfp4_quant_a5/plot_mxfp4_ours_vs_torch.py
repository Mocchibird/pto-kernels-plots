...
def draw_bandwidth(axis, rows):
    ks, ours = series(rows, "ours", 0)  # preallocated: what a caller deploys
    kv, vendor = series(rows, "torch_npu", 1)
    axis.plot(ks, [v / 1000 for v in ours], "-o", color=OURS, lw=2, ms=7, label="ours")
    if vendor:
        axis.plot(
            kv,
...
def draw_ratio(axis, rows):
    ks, ours = series(rows, "ours", 0)  # outputs supplied by the caller
    kv, vendor = series(rows, "torch_npu", 1)
    if not vendor:
        axis.text(
